...
while running:
    ...
    if game_state == START:
        ...
        # ---------------- Draw Clones as Shadow with Animation ----------------
        for clone in clones:
            # Determine if clone is on ground
            clone_on_ground = False
            for plat in platforms:
                if clone["rect"].colliderect(plat) and clone["rect"].bottom <= plat.top + 5:
                    clone_on_ground = True
                    break

            # Animate clone independently
            if not clone_on_ground:
                clone_sprite = jump_up if clone["rect"].y < clone["trail"][-1][1] else jump_down
            else:
                clone["frame_timer"] += 1
            ...
            # Flip if clone is facing left
            if not clone["facing_right"]:
                clone_sprite = pygame.transform.flip(clone_sprite, True, False)

            # Shadow effect
            shadow = clone_sprite.copy()
            shadow.fill((150, 150, 150, 255), special_flags=pygame.BLEND_RGBA_MULT)
